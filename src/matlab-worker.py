...
def start_run(
    ...
    if mode == "function" and not function_name:
        ...
        return None, script_counter
    if mode == "function" and function_name == CAPTURE_FUNCTION:
        immediate_failure(
            engine,
            session_dir,
            run_id,
...
def main() -> int:
    ...
    try:
        if sys.version_info[:2] != (3, 13):
            raise RuntimeError(
                "MATLAB execution requires a Python 3.13 interpreter."
            )
        args = parse_args()
        ...
        if release != EXPECTED_RELEASE:
            raise RuntimeError(
                f"MATLAB {EXPECTED_RELEASE} is required; the configured Engine started {release}."
            )
