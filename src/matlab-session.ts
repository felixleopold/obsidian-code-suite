...
/** Extra Run All allowance beyond the configured execution timeout. */
export const MATLAB_RUN_ALL_EXTRA_MS = STARTUP_TIMEOUT_MS + FIGURE_TIMEOUT_MS;

export type MatlabExecutionStatus = "starting" | "restarting" | "running";

export interface MatlabExecutionCallbacks {
  onStdout?: (data: string) => void;
  onStderr?: (data: string) => void;
  onStatus?: (status: MatlabExecutionStatus) => void;
}
...
function parseWorkerMessage(line: string): WorkerMessage | null {
  ...
  switch (value["type"]) {
    case "ready":
      return typeof value["release"] === "string" ? value as WorkerMessage : null;
    case "stdout":
      if (value["executionDone"] !== undefined && typeof value["executionDone"] !== "boolean") return null;
      // Fall through to the shared stream fields.
    case "stderr":
    ...
    case "done": {
      ...
      const validFigures = Array.isArray(figures) && figures.every((figure) => (
        isRecord(figure) &&
        typeof figure["path"] === "string" &&
        typeof figure["figureIndex"] === "number"
      ));
      return (
...
export class MatlabSessionManager {
  ...
  run(
    ...
    if (!settings.matlabPythonPath.trim()) {
      return this.immediateFailure(
        "Set a Python interpreter with MATLAB Engine installed in CodeSuite settings.",
        callbacks,
      );
    }
    ...
    if (!session) {
      session = { notePath, queue: [], generation: 0, disposed: false };
  ...
  /** Paths with a live Engine or restart marker retained until explicit session cleanup. */
  notePaths(): string[] {
  ...
  private startWorker(
    ...
    if (request) {
      const restarting = session.generation > 0;
      try { request.callbacks?.onStatus?.(restarting ? "restarting" : "starting"); } catch { /* UI-only */ }
      if (restarting) {
        this.appendOutput(
          session,
          request,
          "stderr",
          "MATLAB Engine restarted; the previous workspace was cleared.\n",
        );
      }
    }
    ...
    worker.startupTimer = window.setTimeout(() => {
      ...
      this.failWorker(
        session,
        worker,
        `MATLAB Engine did not start within ${STARTUP_TIMEOUT_MS / 1_000} seconds.`,
        true,
      );
    }, STARTUP_TIMEOUT_MS);
  ...
  private handleMessage(session: NoteSession, worker: MatlabWorker, message: WorkerMessage): void {
    if (message.type === "ready") {
      ...
      const active = session.active;
      if (active) {
        try { active.callbacks?.onStatus?.("running"); } catch { /* UI-only */ }
        this.sendRun(session, worker, active);
      }
      return;
    }
  ...
  private finishActive(session: NoteSession, result: ExecutionResult): void {
    ...
    if (!session.disposed) {
      ...
      if (!session.active && session.queue.length === 0) {
        this.scheduleIdleShutdown(session, request.settings.matlabSessionIdleTimeout);
      }
    }
  }
  ...
  private clearIdleTimer(session: NoteSession): void {
    if (session.idleTimer === undefined) return;
    window.clearTimeout(session.idleTimer);
    session.idleTimer = undefined;
  }

  private scheduleIdleShutdown(session: NoteSession, timeoutMs: number): void {
    this.clearIdleTimer(session);
    if (timeoutMs <= 0) return;
    session.idleTimer = window.setTimeout(() => {
      session.idleTimer = undefined;
      if (
        this.sessions.get(session.notePath) === session &&
        !session.active &&
        session.queue.length === 0
      ) {
        const worker = session.worker;
        if (!worker) return;
        // Retain the session record so the next run can disclose that this
        // note's previous MATLAB workspace expired while it was idle.
        session.worker = undefined;
        void this.shutdownWorker(worker);
      }
    }, timeoutMs);
  }
  ...
  private async shutdownWorker(worker: MatlabWorker): Promise<void> {
    ...
    const closedGracefully = await this.waitForClose(worker, CANCEL_GRACE_MS);
    if (!closedGracefully) {
      this.killWorkerTree(worker);
      await this.waitForClose(worker, 1_000);
    }
    this.cleanupWorker(worker);
  }
  ...
  private waitForClose(worker: MatlabWorker, timeoutMs: number): Promise<boolean> {
