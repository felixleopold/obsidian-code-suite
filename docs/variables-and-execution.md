...
MATLAB is deliberately separate from this replay and variable-sharing model. Its Engine is serialized per note, even when **Shared execution context** is disabled, and retains state until it is cleared or reaches the configured idle timeout. MATLAB fences share the native base workspace, but do not receive `vars`/frontmatter seeds and do not publish values to `$varname` or other languages. See [MATLAB Engine](configuration.md#matlab-engine) for function-fence and figure semantics.
